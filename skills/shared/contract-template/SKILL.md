...
scope: shared
used-by: [planner]
description: The structured contract the Planner produces per task — one immutable, tier-sized spec ([core] sections for a Standard task, [full] added for Domain) that downstream roles measure against. Use when drafting a contract.
---
...
- **One template, sized by tier** (see *Task sizing* in `pipeline-conduct`). Fill the
